...
class _CreateInvoiceScreenState extends State<CreateInvoiceScreen> {
  ...
  Future<void> _saveInvoice() async {
    if (!_formKey.currentState!.validate() || _items.isEmpty) {
      if (_items.isEmpty) Helpers.showErrorSnackBar(context, 'الرجاء إضافة منتج واحد على الأقل');
      return;
    }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // ==== هذا هو التعديل الأهم لحل مشكلة الحقول المخفية ====
            Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildCustomerNameField())),
            const SizedBox(height: 16),
            Card(child: _buildDateField()),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _previousBalanceController, label: 'الحساب السابق', hint: '0', icon: Icons.account_balance_wallet, keyboardType: TextInputType.number, focusNode: _previousBalanceFocus, nextFocus: _amountPaidFocus)))),
              const SizedBox(width: 12),
              Expanded(child: Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _amountPaidController, label: 'المبلغ الواصل', hint: '0', icon: Icons.payment, keyboardType: TextInputType.number, focusNode: _amountPaidFocus, onFieldSubmitted: (_) => _productNameFocus.requestFocus())))),
            ]),
            const SizedBox(height: 24),
            _buildSectionTitle('إضافة منتج'),
            Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildProductNameField())),
            const SizedBox(height: 16),
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(flex: 2, child: Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _quantityController, label: 'الكمية', hint: '1', icon: Icons.shopping_cart, keyboardType: TextInputType.number, focusNode: _quantityFocus, nextFocus: _priceFocus)))),
              const SizedBox(width: 12),
              Expanded(flex: 3, child: Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _priceController, label: 'السعر', hint: '0', icon: Icons.attach_money, keyboardType: TextInputType.number, focusNode: _priceFocus, nextFocus: _itemNotesFocus)))),
            ...
            const SizedBox(height: 16),
            Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _itemNotesController, label: 'ملاحظات المنتج (اختياري)', hint: 'ملاحظات...', icon: Icons.note, maxLines: 2, focusNode: _itemNotesFocus, onFieldSubmitted: (_) => _addItem()))),
            const SizedBox(height: 24),
            if (_items.isNotEmpty) ...[_buildSectionTitle('المنتجات المضافة (${_items.length})'), _buildItemsList(), const SizedBox(height: 24)],
            _buildTotalsCard(),
            const SizedBox(height: 16),
            Card(child: Padding(padding: const EdgeInsets.all(8.0), child: _buildTextField(controller: _notesController, label: 'ملاحظات الفاتورة (اختياري)', hint: 'ملاحظات...', icon: Icons.description, maxLines: 3))),
        ...
        return TextFormField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: controller.text.isNotEmpty
                ...
                : null,
            // جعل الحقل بدون حدود لأنه داخل Card
  ...
  Widget _buildProductNameField() { return Autocomplete<Product>( optionsBuilder: (textEditingValue) async { if (textEditingValue.text.isEmpty) { return const Iterable<Product>.empty(); } await _searchProducts(textEditingValue.text); return _productSuggestions; }, displayStringForOption: (product) => product.name, onSelected: (product) { _productNameController.text = product.name; _priceController.text = product.price.toString(); _quantityFocus.requestFocus(); }, fieldViewBuilder: (context, controller, focusNode, onSubmitted) { _productNameController.text = controller.text; return TextFormField( controller: controller, focusNode: focusNode, decoration: InputDecoration( labelText: 'اسم المنتج *', hintText: 'ابحث أو أدخل اسم منتج جديد', prefixIcon: const Icon(Icons.inventory), suffixIcon: controller.text.isNotEmpty ? IconButton( icon: const Icon(Icons.clear), onPressed: () { controller.clear(); _productNameController.clear(); _priceController.clear(); }, ) : null, border: InputBorder.none ), textInputAction: TextInputAction.next, onFieldSubmitted: (_) => _quantityFocus.requestFocus(), onChanged: (value) { _productNameController.text = value; }, ); }, optionsViewBuilder: (context, onSelected, options) { return Align( alignment: Alignment.topLeft, child: Material( elevation: 4, borderRadius: BorderRadius.circular(8), child: ConstrainedBox( constraints: const BoxConstraints(maxHeight: 200, maxWidth: 300), child: ListView.builder( padding: EdgeInsets.zero, shrinkWrap: true, itemCount: options.length, itemBuilder: (context, index) { final product = options.elementAt(index); return ListTile( title: Text(product.name), subtitle: Text( Helpers.formatCurrency(product.price), style: const TextStyle( color: AppConstants.primaryColor, fontWeight: FontWeight.bold, ), ), onTap: () => onSelected(product), ); }, ), ), ), ); }, ); }
  Widget _buildTextField({ required TextEditingController controller, required String label, required String hint, required IconData icon, TextInputType? keyboardType, int maxLines = 1, FocusNode? focusNode, FocusNode? nextFocus, Function(String)? onFieldSubmitted, }) { return TextFormField( controller: controller, focusNode: focusNode, decoration: InputDecoration( labelText: label, hintText: hint, prefixIcon: Icon(icon), border: InputBorder.none, ), keyboardType: keyboardType, maxLines: maxLines, textInputAction: nextFocus != null || onFieldSubmitted != null ? TextInputAction.next : TextInputAction.done, onFieldSubmitted: (value) { if (onFieldSubmitted != null) { onFieldSubmitted(value); } else if (nextFocus != null) { nextFocus.requestFocus(); } }, ); }
  Widget _buildDateField() { return InkWell( onTap: () async { final date = await showDatePicker( context: context, initialDate: _selectedDate, firstDate: DateTime(2020), lastDate: DateTime(2030), locale: const Locale('ar'), ); if (date != null) { setState(() => _selectedDate = date); } }, child: InputDecorator( decoration: const InputDecoration( labelText: 'تاريخ الفاتورة', prefixIcon: Icon(Icons.calendar_today), border: InputBorder.none ), child: Text( Helpers.formatDate(_selectedDate), style: const TextStyle(fontSize: 16), ), ), ); }
