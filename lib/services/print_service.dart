...
class PrintService {
  ...
  pw.Widget _buildInvoiceHeader(Invoice invoice, pw.Font boldFont) {
    return pw.Container(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(AppConstants.appName, style: pw.TextStyle(font: boldFont, fontSize: 24, color: PdfColors.white)),
          pw.SizedBox(height: 5),
          pw.Text('فاتورة رقم: ${invoice.invoiceNumber}', style: pw.TextStyle(font: boldFont, fontSize: 16, color: PdfColors.white)),
        ],
      ),
    );
  }
      ...
      child: pw.Row(
        ...
        children: [
          pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text('اسم الزبون:', style: pw.TextStyle(font: font, fontSize: 12)),
            pw.SizedBox(height: 5),
            pw.Text(invoice.customerName, style: pw.TextStyle(font: boldFont, fontSize: 14)),
          ]),
          pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text('التاريخ:', style: pw.TextStyle(font: font, fontSize: 12)),
            pw.SizedBox(height: 5),
            pw.Text(Helpers.formatDate(invoice.invoiceDate, useArabic: false), style: pw.TextStyle(font: boldFont, fontSize: 14)),
          ]),
        ],
      ),
    );
  ...
  pw.Widget _buildProductsTable(Invoice invoice, pw.Font font, pw.Font boldFont) {
  ...
  pw.Widget _buildTotals(Invoice invoice, pw.Font font, pw.Font boldFont) {
    return pw.Container(
      padding: const pw.EdgeInsets.all(15),
      decoration: pw.BoxDecoration(
      ...
      child: pw.Column(
        children: [
          _buildTotalRow('مجموع الفاتورة:', invoice.total, font, boldFont),
          if (invoice.previousBalance > 0) ...[
            pw.SizedBox(height: 8),
            _buildTotalRow('الحساب السابق:', invoice.previousBalance, font, boldFont),
          ],
          pw.Divider(thickness: 2, color: PdfColor.fromHex('#10b981')),
          // ==== تم التعديل هنا ====
          _buildTotalRow('الإجمالي الكلي:', invoice.totalWithPrevious, font, boldFont, isMain: true),
          if (invoice.amountPaid > 0) ...[
            pw.SizedBox(height: 8),
            _buildTotalRow('المبلغ الواصل:', invoice.amountPaid, font, boldFont),
            pw.Divider(thickness: 2, color: PdfColor.fromHex('#10b981')),
            _buildTotalRow('المتبقي:', invoice.remainingBalance, font, boldFont, isMain: true),
          ],
        ],
      ),
    );
  ...
  pw.Widget _buildTotalRow(String label, double amount, pw.Font font, pw.Font boldFont, {bool isMain = false}) {
    return pw.Row(
      mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
      children: [
        pw.Text(label, style: pw.TextStyle(font: isMain ? boldFont : font, fontSize: isMain ? 14 : 12)),
        pw.Text(
  ...
  pw.Widget _buildStatementHeader(String customerName, pw.Font boldFont) {
    return pw.Container(
      ...
      child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('كشف حساب', style: pw.TextStyle(font: boldFont, fontSize: 24, color: PdfColors.white)),
        pw.SizedBox(height: 5),
        pw.Text(customerName, style: pw.TextStyle(font: boldFont, fontSize: 18, color: PdfColors.white)),
        pw.SizedBox(height: 5),
        pw.Text('التاريخ: ${Helpers.formatDate(DateTime.now(), useArabic: false)}', style: pw.TextStyle(font: boldFont, fontSize: 12, color: PdfColors.white)),
      ]),
    );
  }

  pw.Widget _buildAccountSummary(double total, double paid, double remaining, pw.Font font, pw.Font boldFont) {
    return pw.Container(
      padding: const pw.EdgeInsets.all(15),
      decoration: pw.BoxDecoration(
        color: PdfColor.fromHex('#ecfdf5'),
      ...
      child: pw.Column(children: [
        pw.Text('ملخص الحساب', style: pw.TextStyle(font: boldFont, fontSize: 16)),
  ...
  pw.Widget _buildInvoicesTable(List<Invoice> invoices, pw.Font font, pw.Font boldFont) {
    return pw.Table.fromTextArray(
      border: pw.TableBorder.all(color: PdfColor.fromHex('#d1fae5')),
      headerStyle: pw.TextStyle(font: boldFont, fontSize: 12),
      headerDecoration: pw.BoxDecoration(color: PdfColor.fromHex('#ecfdf5')),
